...
| 2026-07-26 | LangGraph implemented as a second AgentLoop runtime, native stays default (ADR-006) | Selected via CUSTOS_AGENT_RUNTIME, same run()/run_streaming() contract, no calling-code changes beyond the factory. Hand-writes its own tool-gating node rather than langgraph.prebuilt (ToolNode/create_react_agent execute side-effectful tools with no confirmation gate by default -- demonstrated in tests/test_langgraph_prebuilt_defeats_gate.py, pinned to langgraph==1.2.9). 61/61 deterministic evals and 73/73 including LLM-dependent cases pass on both runtimes, both headline red-team cases exercised (model attempted the unsafe action, hard gate blocked it) not just passed. Zero measured token overhead; benchmarked in docs/benchmarks/agent-runtimes.md. |
| 2026-07-26 | evals.harness always reports all 73 case slots, never a shrinking denominator | Fixed retrieval.py/exfiltration.py/action_gating.py so an LLM-dependent case that does not run (no --llm, or --llm without a key) appears as an explicit SKIP result instead of being silently omitted. Previously the reported total varied by environment (61, 63, or 73) for the identical suite. `make evals` with no key now always reports "Proven: 61 Skipped: 12 Failed: 0", zero API calls, zero cost. |
